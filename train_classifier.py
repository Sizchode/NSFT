...
from diet.mlps.ns_linear2 import NeuroselectiveLinear  # Use the selective linear layer from ns_linear2.py
from diet.ns_transformer5 import NeuroselectiveTransformer5  # Use the pruner from ns_transformer4.py
from diet.tracker6 import NeuronTracker6 as NeuronTracker
from diet.tracker7 import NeuronTracker7 as AblationNeuronTracker

from diet.dataset import get_dataloader
...
import matplotlib
matplotlib.use("Agg")  # 在无显示环境(CCV)下渲染
...
def main():
    ...
    # Run mode
    parser.add_argument("--mode", type=str, default="ns",
                        choices=["ns", "nslora", "lora", "adalora", "loha", "lokr", "baseline","mag_pt", "mag_tp", "wanda_p", "transfer", "cross_task_plot",
                        "ablation","calibration","wanda_tp"],
                        help="Run mode: 'ns' for neuron selection, 'lora' for LoRA, 'baseline' for full finetuning")
    ...
    if args.modality == "image":
        ...
        # ===== [FLOPs] Prepare single-image & baseline counts (ViT) =====
        F_fwd_base      = None  # baseline per-image forward FLOPs
        F_fwd_variant   = None  # variant per-image forward FLOPs
        F_train_variant = None  # variant per-image train FLOPs
        selection_flops = 0     # 固定：baseline forward × (#selected)，仅 NS/NS-LoRA 计入

        _batch0 = next(iter(train_dataloader))
        if isinstance(_batch0, (list, tuple)) and len(_batch0) >= 1 and torch.is_tensor(_batch0[0]):
            _x1 = _batch0[0][:1].to(device)  # (1,3,H,W)
            ...
            _x1 = _batch0["pixel_values"][:1].to(device)
    ...
    if args.mode == "calibration":
        ...
        plt.savefig(f"{base}.pdf", bbox_inches="tight")   # 矢量
        plt.savefig(f"{base}.svg", bbox_inches="tight")   # 矢量
    ...
    # Create active dataset for neuron selection if needed
        ...
        wanda_indices_all = tracker_a.get_active_indices(dataloader=sel_loader)
        ...
        model = nstransformer.transform().to(device)
        ...
        tracker = NeuronTracker(
            model=model,
            tokenizer=tokenizer,                        # Need text model
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(pbar):
            ...
            global_train_samples += (labels.size(0) if args.modality == "image" else 0)
    ...
    if args.mode in ("mag_tp","wanda_tp"):
        ...
        if args.mode == "wanda_tp":
            sel_loader = sample_active_set(non_shuffle_train_dataloader, ratio=args.sample_ratio)
            tracker_a = AblationNeuronTracker(model, topk_ratio=1.0, device=device, verbose=False)
            wanda_all = tracker_a.get_active_indices(dataloader=sel_loader)   # {lname: 索引降序}
            def _is_fc1(lname:str)->bool:
        ...
        # 1) 冻结全部参数，仅解冻输出头与 NeuroselectiveLinear（保持 attention 冻结）
        for p in model.parameters():
            p.requires_grad = False
        trainable = []
        for name, module in model.named_modules():
    ...
    results = {
        ...
        "mode": args.mode,
        "threshold": args.threshold if args.mode == "ns" else None,
        "use_abs_threshold": args.use_abs_threshold if args.mode == "ns" else None,
