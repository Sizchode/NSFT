...
class NSLoraLinear(nn.Module):
    ...
    def __init__(
            self,
            ns_linear: NeuroselectiveLinear,
            r: int = 16,
            lora_alpha: int = 32,
        ...
        Args:
            ns_linear: The NeuroselectiveLinear layer to apply LoRA to
            r: LoRA rank
        ...
        # Get dimensions from ns_linear
        self.in_features = self.ns_linear.active_in_features
        self.out_features = self.ns_linear.active_out_features
        self.original_in_features = self.ns_linear.original_in_features
        self.original_out_features = self.ns_linear.original_out_features
        ...
        for param in self.ns_linear.parameters():
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.ns_linear.in_indices is not None:
            x_selected = torch.index_select(x_flat, 1, self.ns_linear.in_indices)
        else:
            x_selected = x_flat
        ...
        # Apply base layer
        base_output = self.ns_linear.linear(x_selected)

        # Combine outputs
        combined_output = base_output + lora_output

        # Handle output feature scattering if needed
        if self.ns_linear.out_indices is not None:
            ...
            output_flat[:, self.ns_linear.out_indices] = combined_output
        else:
        ...
        if not self.merge_weights:
            if self.r > 0:
                with torch.no_grad():
                    ...
                    self.ns_linear.linear.weight.data += lora_weights
    ...
    def weight(self):
        ...
        if self.merge_weights:
            return self.ns_linear.linear.weight
        else:
            return self.ns_linear.linear.weight + (self.lora_B.weight @ self.lora_A.weight) * self.scaling
    ...
    def __repr__(self):
        return (f"{self.__class__.__name__}("
                ...
                f"bias={self.ns_linear.linear.bias is not None})")